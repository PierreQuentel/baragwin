...
function run_py(module_contents, path, module, compiled) {
    ...
    try{
        ...
        js = "var $module = (function(){\n" + js + "return $locals_" +
            module.__name__.replace(/\./g, "_") + "})(__BARAGWIN__)\n" +
            "return $module"
    ...
    try{
        ...
        if($B.debug > 0){console.log("line info " + __BARAGWIN__.line_info)}
        throw err
    }
}
...
var finder_VFS = {
    ...
    exec_module : function(cls, modobj) {
        ...
        var path = $B.baragwin_path + "Lib/" + modobj.__name__
    ...
    find_spec : function(cls, fullname, path, prev_module) {
        ...
        if(stored){
            ...
            return new_spec({
                ...
                origin : is_builtin? "built-in" : "baragwin_stdlib",
...
var _sys_paths = [[$B.script_dir + "/", "py"],
                  [$B.baragwin_path + "Lib/", "py"],
                  [$B.baragwin_path + "Lib/site-packages/", "py"],
                  [$B.baragwin_path + "libs/", "js"]]
...
$B.$__import__ = function(mod_name, globals, locals, fromlist, level){
    ...
    // in function baragwin(), based on the options passed to this function.
    // The available meta paths are :
    // - finder_VFS : search in the Virtual File System ; used if
    //   baragwin_stdlib.js or baragwin_modules.js was loaded in the page
   ...
   if(globals.$jsobj && globals.$jsobj.__file__){
       ...
       if((file.startsWith($B.baragwin_path + "Lib/") &&
               ! file.startsWith($B.baragwin_path + "Lib/site-packages/")) ||
               file.startsWith($B.baragwin_path + "libs/") ||
               file.startsWith("VFS.")){
           from_stdlib = true
       }
   }
 ...
 *
 * The function sets __BARAGWIN__.modules[mod_name] and
 * __BARAGWIN__.imported[mod_name] to an object representing the
...
// List of path hooks. It is changed by function baragwin() depending
// on the options passed
$B.$path_hooks = [url_hook]

// List of finders, also used by baragwin()
