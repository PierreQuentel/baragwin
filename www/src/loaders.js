...
if($B.VFS_timestamp && $B.VFS_timestamp > $B.timestamp){
    // A VFS created by python -m baragwin --modules has its own
    // timestamp. If it is after the one in baragwin.js, use it
...
function idb_load(evt, module){
    ...
    // baragwin_stdlib, use the precompiled Javascript.
    // Otherwise, get the source code from baragwin_stdlib.js.
    ...
    if(res === undefined || res.timestamp != $B.timestamp){
        // Not found or not with the same date as in baragwin_stdlib.js:
        // search in VFS
...
function store_precompiled(module, js, source_ts, imports, is_package){
    ...
    var db = $B.idb_cx.result,
        ...
        data = {"name": module,
            ...
            "imports": imports,
            "origin": origin,
            "timestamp": __BARAGWIN__.timestamp,
            "source_ts": source_ts,
    ...
    idb_cx.onsuccess = function(){
        ...
        if(!db.objectStoreNames.contains("modules")){
            ...
            store.openCursor().onsuccess = function(evt){
                ...
                if(cursor){
                    record = cursor.value
                    // A record is valid if the _Baragwin engine timestamp is
                    ...
                    if(record.timestamp == $B.timestamp){
                        if(!$B.VFS || !$B.VFS[record.name] ||
                            ...
                            // Load in __BARAGWIN__.precompiled
...
var inImported = $B.inImported = function(module){
    ...
    }else if(__BARAGWIN__.VFS && __BARAGWIN__.VFS.hasOwnProperty(module)){
        var elts = __BARAGWIN__.VFS[module]
