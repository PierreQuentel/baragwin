(function() {
    ...
    // read data-baragwin-options (or data-loader-options) if it exists..
    var _loader=parse_options(script.getAttribute("data-loader-options"))
    var _baragwin=parse_options(script.getAttribute("data-baragwin-options"))

    //figure out "root" path so we know where baragwin source file(s) are located
    ...
    function import_file(path, pos) {
       ...
       if (pos >= _files.length) {
          window.onload=function() {baragwin(_baragwin)}
          return
       }
    ...
    if (_loader.dist == true) {
       var _s = document.createElement('script');
       _s.src = _path + "baragwin_dist.js";
       _s.setAttribute('type', 'text/javascript')
       _s.onload= function() { baragwin(_baragwin) };
       document.head.appendChild(_s);
       return
    }

    var _files=[]
    if (_loader.debug === undefined || _loader.debug==0) {  // just load baragwin.js
       _files.push("baragwin")
    } else {
      
       //import_file(_path,0)
        var py_files=['baragwin_builtins', 'version_info', 'identifiers_re',
