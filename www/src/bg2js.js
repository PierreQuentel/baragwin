...
var $CallCtx = $B.parser.$CallCtx = function(context){
    ...
    this.to_js = function(){
        ...
        for(var i = 0, len = this.tree.length; i < len; i++){
            ...
            if(arg.type == "call_arg"){
                switch(arg.tree[0].type){
                    case "expr":
                        args.push(i + ':' +arg.tree[0].to_js())
                        break
                    case "kwarg":
                        var kw = arg.tree[0].tree
                        args.push(kw[0].value + ':' + kw[1].to_js())
                        break
                }
            }
        }
        return this.func.to_js() + "({" + args.join(", ") + "})"
        /*
        this.js_processed = true
        ...
        if(this.func !== undefined) {
            ...
            var default_res = func_js + args_str
...
var $DefCtx = $B.parser.$DefCtx = function(context){
    ...
    this.to_js = function(func_name){
        ...
        return 'locals.' + this.name + ' = function(args)'
    }
}
...
var $TryCtx = $B.parser.$TryCtx = function(context){
    ...
    this.transform = function(node, rank){
        ...
        // Store exception as __BARAGWIN__.cuex (for "current exception")
